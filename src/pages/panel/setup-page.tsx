...
export const SetupPage = (): React.JSX.Element => {
  ...
  if (status.data?.nextStep === 'complete') return <section className="grid max-w-md gap-4 panel-surface p-6"><h1 className="page-title">Panel already set up</h1><p className="text-sm text-ink-muted">An administrator already exists for this panel.</p><Button onClick={returnToSignIn}>Go to sign in</Button></section>;

  return (
    <section className="mx-auto grid w-full max-w-md gap-6 panel-surface p-6" aria-labelledby="setup-heading">
      <div><h1 className="page-title" id="setup-heading">Create first administrator</h1><p className="mt-2 text-sm text-ink-muted">Enter the setup token supplied by the panel operator. It is cleared as soon as this request finishes.</p></div>
      {error ? <Alert kind="error" role="alert">{error}</Alert> : null}
      <form className="grid gap-4" onSubmit={(event) => void submit(event)}>
        <Field id="setup-email" label="Email" type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} required />
        <Field id="setup-username" label="Username" autoComplete="username" value={username} onChange={(event) => setUsername(event.target.value)} required />
        <label className="grid gap-2" htmlFor="setup-token">
          <span className="font-bold text-ink">Setup token</span>
          <input ref={tokenRef} id="setup-token" type="password" autoComplete="off" required className="mp-field w-full px-4 py-3 font-mono text-sm text-ink placeholder:text-ink-faint focus:border-accent focus:outline-none" />
        </label>
        <Field id="setup-password" label="Administrator password" type="password" autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} required />
        <Button type="submit" loading={busy}>Create administrator</Button>
      </form>
    </section>
  );
};
